...
router.post("/", authenticateToken, (req, res) => {
  ...
  console.log(user);
  if (!title || !body)
    res.status(404).send({ msg: "You did not send all your details" });
  let sql = `INSERT INTO posts (post_title, post_body, post_date, post_author) VALUES ('${title}', '${body}', '${d.getFullYear()}-${
    d.getMonth() + 1
  }-${d.getDate()}', ${user.user_id})`;
